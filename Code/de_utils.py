...
def generate_correction_report(analysis_folder):
    """
    Working function for generating correction report. The report consists of (i) scatter plots of original and corrected trajectories, (ii) circle quality of original and corrected trajectories and (iii) a montage of correction snapshots (which are generated during the correction). For a sketch, see <here `https://drive.google.com/open?id=1DMyNm-0Fw7W_YzETGDrRujoAvRoGBdUB&authuser=liux3141%40umn.edu&usp=drive_fs`>_.

    The analysis folder is designed as the following:

    ```
    analysis_folder/
        hough_params.json  ..  (to initialize track_droplets.py)
        finding.csv  ..  (product of track_droplets.py)
        tracking.csv  ..  (link from finding.csv)
        correction_params.json  ..  (to initialize correction.py)
        corrected_tracking.csv  ..  (product of correction.py)
        overlays/  ..  (generated by track_droplets.py, red circle denotes detected circle)
        correction-snapshots/  ..  (generated by correction.py, red/green denotes original/corrected)
    ```

    :param analysis_folder: folder that contains tracknig data, with the structure specified above.

    .. rubric:: Edit

    * Jun 23, 2022 -- Change input arguments to original_traj, corrected_traj, img_folder
    * Mar 14, 2022 -- (i) docstring (ii) remove redundant input params, only keep analysis_folder since all files can be found in there. I just need to specify the names correctly.
    """

    # infer data directories
    name = "DE#" + os.path.split(analysis_folder)[1]
    correction_snapshot_folder = os.path.join(analysis_folder, "correction-snapshots")
    tracking = pd.read_csv(os.path.join(analysis_folder, "tracking.csv"))
    corrected_tracking = pd.read_csv(os.path.join(analysis_folder, "corrected_tracking.csv"))

    # plot preparation
    fig = plt.figure(figsize=(16, 9), dpi=300)

    subfigs = fig.subfigures(2, 1, height_ratios=(1, 8))

    # title 
    ax = subfigs[0].add_axes([0,0,1,1])
    ax.annotate(name + " tracking correction report", (0, .5), xycoords="axes fraction", fontsize=40, verticalalignment="center")
    ax.axis("off")

    # second row
    subsubfigs = subfigs[1].subfigures(1, 2, width_ratios=(10, 6))

    # second row first column
    subsubsubfigs = subsubfigs[0].subfigures(2, 1, height_ratios=(5, 3))

    # montage
    ax = subsubsubfigs[0].add_axes([0,0,1,1])
    l = readdata(correction_snapshot_folder, "jpg")
    if len(l) > 32: l = l[:32]
    ss_list = []
    for num, i in l.iterrows():
        ss_list.append(io.imread(i.Dir))
    arr_in = np.stack(ss_list)
    mon = util.montage(arr_in, channel_axis=3, grid_shape=(4, 8), padding_width=3)
    ax.imshow(mon)
    ax.axis("off")

    # quality plot
    ax = subsubsubfigs[1].subplots() 
    ax.plot(corrected_tracking.frame, corrected_tracking.original_quality, color="red", alpha=0.3)
    ax.plot(corrected_tracking.frame, corrected_tracking.corrected_quality, color="green", alpha=0.3)

    ax.plot(corrected_tracking.frame, gaussian_filter1d(corrected_tracking.original_quality, sigma=5), color="red")
    ax.plot(corrected_tracking.frame, gaussian_filter1d(corrected_tracking.corrected_quality, sigma=5), color="green")
    ax.set_xlabel("frame")
    ax.set_ylabel("quality")

    # scatter plot
    ax = subsubfigs[1].subplots(2, 1)
    ax[0].scatter(tracking.x, tracking.y, s=1, color="red")
    ax[0].scatter(corrected_tracking.x, corrected_tracking.y, s=1, color="green")
    ax[0].set_xlabel("x")
    ax[0].set_ylabel("y")

    ax[1].scatter(tracking.x, tracking.y, s=1, color="red")
    ax[1].scatter(corrected_tracking.x, corrected_tracking.y, s=1, color="green")
    ax[1].set_xlabel("x")
    ax[1].set_ylabel("y")
    ax[1].axis("equal")


    fig.savefig(os.path.join(analysis_folder, "tracking_correction_report.jpg"))
